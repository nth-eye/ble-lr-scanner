...
{
    auto slow_1 = crc_slow<T, poly, init, xorout, refin, refout>({reinterpret_cast<const byte*>(test_data), test_size});
    auto fast_1 = crc_fast<T, poly, init, xorout, refin, refout>({reinterpret_cast<const byte*>(test_data), test_size});
    auto slow_2 = [] () {
        T ret = 0;
        ret = crc_slow_init<T, init>();
        ret = crc_slow_feed<T, poly, refin>(ret, {reinterpret_cast<const byte*>(test_data), test_part});
        ret = crc_slow_feed<T, poly, refin>(ret, {reinterpret_cast<const byte*>(test_data) + test_part, test_size - test_part});
        ret = crc_slow_stop<T, xorout, refout>(ret);
        return ret;
    }();
    auto fast_2 = [] () {
        T ret = 0;
        ret = crc_fast_init<T, init>();
        ret = crc_fast_feed<T, poly, refin>(ret, {reinterpret_cast<const byte*>(test_data), test_part});
        ret = crc_fast_feed<T, poly, refin>(ret, {reinterpret_cast<const byte*>(test_data) + test_part, test_size - test_part});
        ret = crc_fast_stop<T, xorout, refout>(ret);
        return ret;
    }();
