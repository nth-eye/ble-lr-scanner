...
#include <cmath>
#include "nth/cbor/enc.h"

namespace nth {
namespace {

void check(cbor::const_view res, std::initializer_list<uint8_t> exp)
{
...
{
    codec.encode_prim(cbor::prim_false);
    codec.encode_prim(cbor::prim_true);
    codec.encode_prim(cbor::prim_null);
    codec.encode_prim(cbor::prim_undefined);
    codec.encode_prim(cbor::prim_t(16));
    codec.encode_prim(cbor::prim_t(255));
...
{
    codec.encode(cbor::prim_false);
    codec.encode(cbor::prim_true);
    codec.encode(cbor::prim_null);
    codec.encode(cbor::prim_undefined);
    codec.encode(cbor::prim_t(16));
    codec.encode(cbor::prim_t(255));
    codec.encode(false);
    codec.encode(true);
...
TEST_F(Encode, ExplicitData)
{
    ...
    codec.encode_data(cbor::span{});
    codec.encode_data(cbor::span{pld});
...
TEST_F(Encode, ImplicitData)
{
    ...
    codec.encode(cbor::span{});
    codec.encode(cbor::span{pld});
...
{
    codec.encode(cbor::idat);
    codec.encode({0x01, 0x02});
    codec.encode({0x03, 0x04, 0x05});
    codec.encode(cbor::stop);
...
TEST_F(Encode, Errors)
{
    ...
    ASSERT_EQ(cbor::err::no_memory, codec.encode(dummy));

    for (int i = 24; i <= 31; ++i)
        ASSERT_EQ(cbor::err::invalid_simple, codec.encode(cbor::prim_t(i)));
