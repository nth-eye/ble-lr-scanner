#ifndef NTH_UTIL_TIME_H
#define NTH_UTIL_TIME_H
...
/**
 * @brief Numbers of days within months.
 * 
 */
inline constexpr int64_t days[] = {
    31, 28, 31, 30, 31, 30, 
    31, 31, 30, 31, 30, 31,
};
...
{
    return m_exec_time<N>(std::forward<Args>(args)...) / N;
}
...
constexpr tm compilation_tm()
{
    ...
    constexpr auto tokens = str_split<6 + spacebar>(timestr, " :");
    ...
    utc.tm_mday = to_integer(tokens[spacebar + 1]);
    utc.tm_year = to_integer(tokens[spacebar + 2]) - 1900;
    utc.tm_hour = to_integer(tokens[spacebar + 3]);
    utc.tm_min = to_integer(tokens[spacebar + 4]);
    utc.tm_sec = to_integer(tokens[spacebar + 5]);
